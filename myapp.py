...
def user_input_features():
        ...
        data = {'fixed acidity': fixed_acidity,
              ...
              'total_sulfur_dioxide':t_sulf_diox,
              'pH':ph,
              'alcohol':alcohol,
                'sulphates':sulphates }
...
if st.button("Predict"):
        ...
        if result == 0:
            st.success('Your wine is bad ')
            prediction = classifier.predict(df)#prediction_proba = classifier.predict_proba(df)
            st.subheader('Prediction')
            st.write(prediction)

            st.subheader('Prediction Probability')
            prediction_proba = classifier.predict_proba(df)#prediction_proba = classifier.predict_proba(df)

            st.write(prediction_proba)

        else:
            st.success('Your wine is good ')
